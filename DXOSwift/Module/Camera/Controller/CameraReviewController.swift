        ...
        DXOService.cameraReview(page: 1) {[weak self] (inObject, inError) in
            if inError != nil || inObject == nil {
                DispatchQueue.main.async {
                    ...
                    let messageView = try? self?.view.toastViewForMessage("please try again later", title: "request failed", image: nil, style: ToastStyle())
                }
                return
            }else if self == nil {
    ...
    func footerRefresh(){
        DXOService.cameraReview(page: page+1) {[weak self] (inReviews, inError) in
            if inError != nil || inReviews == nil {
                ...
                DispatchQueue.main.async {
                    let _ = try? self?.view.toastViewForMessage("please try again", title: "Failed", image: nil, style: ToastStyle.init())
